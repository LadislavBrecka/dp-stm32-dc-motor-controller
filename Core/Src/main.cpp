...
uint32_t ident_exp_index = 0;
int16_t	ident_exp_sp[2048] = {
...
uint32_t reg_exp_index = 0;
int16_t	reg_exp_sp[4096] = {
...
SimData automatic_system_loop()
{
	...
	{
		if (ident_exp_index < (sizeof(ident_exp_sp) / sizeof(* ident_exp_sp)))
		{
			u_speed = ident_exp_sp[ident_exp_index++];
			pwm_stride = u_speed;
			// identification
...
//		reg_coefs[2] = 0.49722908302645674;
	...
	{
		if (reg_exp_index < (sizeof(reg_exp_sp) / sizeof(* reg_exp_sp)))
		{
			w_pos = reg_exp_sp[reg_exp_index++];
			u_speed = reg->step(w_pos, y_speed, y_pos).u;
			pwm_stride = u_speed;
		}
		else
		{
...
//	data.y_speed *= (2*M_PI / 12.0);
	...
	HAL_UART_Transmit(&huart2, (uint8_t*)"S", sizeof("S"), 2);
	HAL_UART_Transmit(&huart2, (uint8_t *)buffer, sizeof(data), 200);
	HAL_UART_Transmit(&huart2, (uint8_t*)"Z", sizeof("Z"), 2);
}
